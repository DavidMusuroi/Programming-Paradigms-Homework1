...
(define (queue-empty? q)
  (if (not (equal? (queue-size-l q) 0))
    #f
    (if(equal? (queue-size-r q) 0)
      #t
      #f)))
...
(define (dequeue q)
  (if (<= (queue-size-l q) (queue-size-r q))
    (make-queue (rotate (stream-rest(queue-left q)) (queue-right q) empty-stream) '() (+ (- (queue-size-l q) 1) (queue-size-r q)) 0)
    (struct-copy queue q [left (stream-rest(queue-left q))] [right (queue-right q)] [size-l (- (queue-size-l q) 1)] [size-r (queue-size-r q)])))
